...
# Calculate the total number of unique transactions for each brand for TW, LW, and LY
for brand in all_brands:
    unique_orders_tw = 0
    unique_orders_lw = 0
    unique_orders_ly = 0

    # Calculate TW unique orders
    if brand in brands_tw:
        brand_sales_tw = filtered_sales_tw[filtered_sales_tw['Brand'] == brand]
        unique_orders_tw = brand_sales_tw['Order No.'].nunique()

    # Calculate LW unique orders
    if brand in brands_lw:
        brand_sales_lw = filtered_sales_lw[filtered_sales_lw['Brand'] == brand]
        unique_orders_lw = brand_sales_lw['Order No.'].nunique()

    # Calculate LY unique orders
    if brand in brands_ly:
        brand_sales_ly = filtered_sales_ly[filtered_sales_ly['Brand'] == brand]
        unique_orders_ly = brand_sales_ly['Order No.'].nunique()

    # Calculate vs LW
    if unique_orders_lw > 0:
        vs_lw = ((unique_orders_tw - unique_orders_lw) / unique_orders_lw) * 100
    else:
        vs_lw = float('inf') if unique_orders_tw > 0 else 0

    # Calculate vs LY
    if unique_orders_ly > 0:
        vs_ly = ((unique_orders_tw - unique_orders_ly) / unique_orders_ly) * 100
    else:
        vs_ly = float('inf') if unique_orders_tw > 0 else 0
...
with pd.ExcelWriter('WTD.xlsx', mode='a', engine='openpyxl', if_sheet_exists='replace') as writer:
    result.to_excel(writer, sheet_name='Gross Transactions', index=False)
